...
def get_dataset():
    ...
    records_with_country_id = get_records()

    records = []
    for raw_record in records_with_country_id:
        country = country_id_to_country[raw_record[1]][0]

        if country is not 'China':
            country = "Other"

        records.append((raw_record[0], country))
    ...
    return ['China', 'Other'], examples, labels
